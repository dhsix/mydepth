#!/usr/bin/env python3
"""
GAMUS nDSM Model Prediction Visualization Script - Individual Sample Version
Generate separate visualization for each sample
"""
...
def create_test_dataset(data_dir, args, logger):
    ...
    train_dataset = GAMUSDataset(
        ...
        height_filter={'min_height': args.min_height, 'max_height': args.max_height}
)
...
def create_individual_visualization(result, save_dir, logger, global_min=None, global_max=None, max_error=None):
    """Create visualization for a single sample"""
    sample_idx = result['index']
    logger.info(f"Creating visualization for sample {sample_idx + 1}...")
    ...
    try:
        ...
        # Calculate display range for this sample if not provided globally
        if global_min is None or global_max is None:
            global_min = min(result['target_real'].min(), result['prediction_real'].min())
            global_max = max(result['target_real'].max(), result['prediction_real'].max())
        
        if max_error is None:
            max_error = min(20, np.percentile(result['error'], 95))
        ...
        ax.imshow(image)
        ax.set_title(f'Sample {sample_idx + 1}\nInput Image', fontsize=12, fontweight='bold')
        ax.axis('off')
        ...
        ax.set_title(f'Ground Truth nDSM\nRange: [{target_real.min():.1f}, {target_real.max():.1f}]m', 
        ...
        ax.set_title(f'Predicted nDSM\nRange: [{prediction_real.min():.1f}, {prediction_real.max():.1f}]m', 
                    fontsize=12, fontweight='bold')
        ax.axis('off')
        ...
        ax.set_title(f'Absolute Error\nMAE: {result["mae"]:.2f}m | RMSE: {result["rmse"]:.2f}m', 
                    fontsize=12, fontweight='bold')
        ax.axis('off')
        ...
        stats_text = f"""Sample {sample_idx + 1} Detailed Statistics
...
Error Analysis:
  ...
  99th Percentile: {np.percentile(error, 99):.2f} m
  
Accuracy Metrics:
  ±1m: {np.mean(error <= 1.0) * 100:.1f}%
  ±2m: {np.mean(error <= 2.0) * 100:.1f}%
  ±5m: {np.mean(error <= 5.0) * 100:.1f}%"""
        
        ax.text(0.05, 0.95, stats_text, transform=ax.transAxes, fontsize=10,
               verticalalignment='top', fontfamily='monospace',
               bbox=dict(boxstyle="round,pad=0.5", facecolor="lightblue", alpha=0.8))
        
        plt.suptitle(f'GAMUS nDSM Prediction Results - Sample {sample_idx + 1}', 
                    fontsize=16, fontweight='bold', y=0.98)
        plt.tight_layout()
        
        # Save individual visualization
        sample_path = os.path.join(save_dir, f'sample_{sample_idx + 1:03d}_prediction.png')
        plt.savefig(sample_path, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        
        logger.info(f"Sample {sample_idx + 1} visualization saved: {sample_path}")
        
        return sample_path
        
    except Exception as e:
        logger.error(f"Failed to create visualization for sample {sample_idx + 1}: {e}")
        import traceback
        traceback.print_exc()
        return None

def create_all_individual_visualizations(results, save_dir, logger):
    """Create individual visualizations for all samples"""
    logger.info("Creating individual visualizations for all samples...")
    
    if not results:
        logger.error("No valid prediction results")
        return []
    
    # Calculate global display ranges for consistency
    ...
    for result in tqdm(results, desc="Creating individual visualizations"):
        sample_path = create_individual_visualization(
            result, save_dir, logger, global_min, global_max, max_error
        )
        if sample_path:
            saved_paths.append(sample_path)
    
    logger.info(f"Successfully created {len(saved_paths)} individual visualizations")
    return saved_paths

def create_summary_visualization(results, save_dir, logger):
    """Create a summary visualization showing all samples in a grid"""
    logger.info("Creating summary grid visualization...")
    ...
    try:
        ...
        cols = min(5, n_samples)  # Maximum 5 columns
        ...
        for i, result in enumerate(results):
            ...
            prediction_real = result['prediction_real']
            im = ax.imshow(prediction_real, cmap=plt.cm.terrain, vmin=global_min, vmax=global_max)
        ...
        plt.suptitle('GAMUS nDSM Prediction Results - Summary Grid', 
                    fontsize=16, fontweight='bold')
        plt.tight_layout()
        
        # Save summary visualization
        summary_path = os.path.join(save_dir, 'summary_grid_predictions.png')
        plt.savefig(summary_path, dpi=200, bbox_inches='tight', facecolor='white')
        plt.close()
        
        logger.info(f"Summary grid visualization saved: {summary_path}")
...
def create_summary_statistics(results, save_dir, logger):
    ...
    try:
        ...
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("GAMUS nDSM Model Individual Sample Prediction Results Summary\n")
            f.write("=" * 65 + "\n\n")
            ...
            f.write("-" * 50 + "\n")
            for i, result in enumerate(results):
                f.write(f"Sample {result['index']+1:3d}: MAE={result['mae']:.3f}m, RMSE={result['rmse']:.3f}m\n")
            
            f.write("\nGenerated Files:\n")
            f.write("-" * 50 + "\n")
            f.write("Individual sample visualizations: sample_XXX_prediction.png\n")
            f.write("Summary grid visualization: summary_grid_predictions.png\n")
            f.write("Error analysis: error_analysis.png\n")
            f.write("This summary: prediction_summary.txt\n")
...
def main():
    parser = argparse.ArgumentParser(description='GAMUS nDSM Model Individual Sample Prediction Visualization')
    ...
    log_file = os.path.join(args.save_dir, f'individual_visualization_log_{timestamp}.log')
    logger = setup_logger(log_file)
    
    logger.info("=" * 80)
    logger.info("GAMUS nDSM Model Individual Sample Prediction Visualization")
    logger.info("=" * 80)
    ...
    try:
        ...
        # Create individual visualizations for each sample
        logger.info("=" * 60)
        logger.info("Creating individual sample visualizations...")
        saved_paths = create_all_individual_visualizations(results, args.save_dir, logger)
        
        # Create summary visualizations if requested
        if args.create_summary:
            logger.info("=" * 60)
            logger.info("Creating summary visualizations...")
            create_summary_visualization(results, args.save_dir, logger)
        
        # Create error analysis if requested
        if args.create_analysis:
            logger.info("=" * 60)
            logger.info("Creating error analysis...")
            create_error_analysis(results, args.save_dir, logger)
        
        # Create summary statistics
        create_summary_statistics(results, args.save_dir, logger)
        
        logger.info("=" * 60)
        logger.info("Individual visualization completed successfully!")
        logger.info(f"Results saved in: {args.save_dir}")
        logger.info(f"Generated {len(saved_paths)} individual sample visualizations")
        ...
        if args.create_summary:
            logger.info(f"  - summary_grid_predictions.png")
        if args.create_analysis:
            logger.info(f"  - error_analysis.png")
        logger.info(f"  - prediction_summary.txt")
        ...
        logger.info(f"  Best MAE: {min(r['mae'] for r in results):.3f}m (Sample {min(results, key=lambda x: x['mae'])['index'] + 1})")
        logger.info(f"  Worst MAE: {max(r['mae'] for r in results):.3f}m (Sample {max(results, key=lambda x: x['mae'])['index'] + 1})")
